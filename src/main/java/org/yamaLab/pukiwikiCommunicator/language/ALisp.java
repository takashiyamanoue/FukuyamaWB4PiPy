...
public class ALisp extends java.lang.Object implements Runnable
{
    ...
    public void initFunctionDispatcher()
    {
         ...
         functionDispatcher.put(sym_and, new Fun_and(this));
         functionDispatcher.put(sym_or, new Fun_or(this));
         functionDispatcher.put(sym_not, new Fun_not(this));
    ...
    public void initSymbols()
    {
        ...
        sym_and    =recSymbol("and");
        sym_or     =recSymbol("or");
        sym_not    =recSymbol("not");
    }
    ...
    public Symbol sym_and;
    public Symbol sym_or;
    public Symbol sym_not;
    ...
    public LispObject preEval(LispObject s, LispObject env)
    {
        ...
        else
        {
            ...
            if(!Null(rtn)) return rtn;
        		...
        		gui.parseCommand("println error at defEXt<preEval, println "+e.toString());
        		System.out.println("error at defEXt<preEval, println "+e.toString());
...
//            return cons(cons(key,cons(value,nilSymbol)),env);
    ...
    public LispObject and(LispObject x, LispObject y)
    {
    	if(Null(x)) return nilSymbol;
    	if(Null(y)) return nilSymbol;
        else        return tSymbol;
    }   
    public LispObject or(LispObject x, LispObject y)
    {
    	if(!Null(x)) return tSymbol;
    	if(!Null(y)) return tSymbol;
        else        return nilSymbol;
    }   
    public LispObject not(LispObject x)
    {
    	if(Null(x)) return tSymbol;
        else        return nilSymbol;
    }            
    ...
    public void plist_n(String s, LispObject x, int n)
    {
...
//        	}
//        	catch(Exception e){
//        		gui.parseCommand("error at apply, println "+e.toString());
//        		plist("s=",proc);
//        		Thread.dumpStack();        		
//        		return nilSymbol;        		
//        	}
...
//           plist("eval return ...",rtn);
        ...
        //String o=print.print(form);
        // System.out.println("eval "+o+" st="+stime+" et="+etime+" rt="+(etime-stime)); // for debug, 2021 12/03 15:28
        return rtn;
